...
registerCommand(commandInformation, (origin, targetPlayerName) => {
  ...
  if (c?.id) {
    ...
    if(!isOwner) {
      ...
      return player.sendMessage(`§cYou can't grant a permission you don't have yourself.`)
    }
  ...
  player.sendMessage(`§aGranted ${targetPlayerName === "all" ? "the public" : targetPlayerName} permission to manage permissions in ${c?.id ? "this claim." : "all your claims."}`)
  db.store("land", lands);
  return {
    status: 0
  }
